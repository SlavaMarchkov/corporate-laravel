<div id="content-blog" class="content group">
    ...
    @foreach ($articles as $article)
        <div class="hentry hentry-post blog-big group">
            ...
            <div class="meta group">
                ...
                <p class="comments"><span><a href="{{ route('articles.show', ['alias' => $article->alias]) }}#respond" title="Comment on {{ $article->title }}">{{ count($article->comments) ? count($article->comments) : '0' }} {{ Lang::choice('ru.comments', count($article->comments)) }}</a></span></p>
            </div>
            <!-- post content -->
    ...
    @else
        <h3>{{ Lang::get('ru.none_articles') }}</h3>
    @endif
</div>